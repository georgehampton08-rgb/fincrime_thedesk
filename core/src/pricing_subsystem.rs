...
impl PricingSubsystem {
    ...
    fn initialize_product_state(&mut self, tick: Tick) -> SimResult<()> {
        for product in self.config.products.values() {
            ...
            self.store
                .insert_product_state(&self.run_id, &state, tick)?;
    ...
    fn validate_fee(&self, fee_type: &str, new_value: f64) -> Result<Option<String>, String> {
        ...
        if new_value < constraint.min_value || new_value > constraint.max_value {
            return Err(format!(
                ...
                fee_type, constraint.min_value, constraint.max_value, constraint.hard_limit_reason
            ));
        }
    ...
    fn process_fee_change(
        ...
        self.store
            .update_product_fee(&self.run_id, product_id, fee_type, new_value, tick)?;
    ...
    fn update_udaap_score(&self, fee_type: &str, new_value: f64, tick: Tick) -> SimResult<()> {
        let formula = match self.config.impact_formulas.get(fee_type) {
            Some(f) => f,
            None => return Ok(()),
        };

        let threshold = formula
            .parameters
        ...
        for event in events_in {
            if let SimEvent::PlayerCommandReceived { command_id, .. } = event {
                match self.store.get_player_command(&self.run_id, command_id) {
                    Ok(Some(PlayerCommand::SetProductFee {
                        product_id,
                        fee_type,
                        new_value,
                    })) => {
                        let events =
                            self.process_fee_change(&product_id, &fee_type, new_value, tick)?;

...
fn initial_population_generates_correct_count() {
    ...
    let count = engine
        ...
        .unwrap();
    assert_eq!(count, 50, "Expected 50 test customers, got {count}");
}

#[test]
fn transactions_generated_every_tick_after_tick_0() {
    let mut engine = SimEngine::build_test("txn-gen-test".into(), 99).unwrap();
    engine.run_ticks(5).unwrap();

    let tick1_count = engine.store.txn_count_for_tick("txn-gen-test", 1).unwrap();
    assert!(tick1_count > 0, "Expected transactions at tick 1, got 0");

    let tick0_count = engine.store.txn_count_for_tick("txn-gen-test", 0).unwrap();
    assert_eq!(
        tick0_count, 0,
...
fn payroll_fires_on_biweekly_boundary() {
    let mut engine = SimEngine::build_test("payroll-test".into(), 7).unwrap();
    engine.run_ticks(14).unwrap();

    // At tick 14, payroll credits should appear
    let payroll_count = engine
        .store
...
fn transaction_amounts_follow_pareto_shape() {
    ...
    let mut engine = SimEngine::build_test("pareto-test".into(), 123).unwrap();
    engine.run_ticks(90).unwrap();

    let amounts = engine.store.all_txn_amounts("pareto-test").unwrap();
...
fn determinism_holds_with_customers_and_transactions() {
    ...
    let mut engine_a = SimEngine::build_test(format!("det-full-a-{SEED}"), SEED).unwrap();
    let mut engine_b = SimEngine::build_test(format!("det-full-b-{SEED}"), SEED).unwrap();

    engine_a.run_ticks(30).unwrap();
    engine_b.run_ticks(30).unwrap();

    let count_a = engine_a
        .store

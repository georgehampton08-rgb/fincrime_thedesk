...
fn pnl_computed_on_quarterly_boundaries() {
    let mut engine = SimEngine::build_test(
        "pnl-boundary-test".into(), 42
    ).unwrap();

    engine.run_ticks(91).unwrap();

    let pnl_count = engine
        .store_pnl_count("pnl-boundary-test")
        .unwrap();

    assert_eq!(
        pnl_count, 1,
        "Expected 1 P&L snapshot at tick 90"
    );
}
...
fn multiple_quarters_produce_multiple_snapshots() {
    ...
    assert_eq!(
        snapshots.len(), 4,
        "Expected 4 quarterly snapshots"
    );

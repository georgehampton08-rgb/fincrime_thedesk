...
fn fee_change_within_limits_succeeds() {
    ...
    let state = engine.store_product_state("fee-change-test", "basic_checking").unwrap();
...
fn fee_change_above_hard_limit_rejected() {
    ...
    let state = engine.store_product_state("hard-limit-test", "basic_checking").unwrap();
...
fn fee_change_above_soft_limit_generates_warning() {
    ...
    // Fee should change
    let state = engine.store_product_state("soft-limit-test", "basic_checking").unwrap();
    ...
    assert!(udaap > 0.0, "UDAAP score should increase above soft limit; got {udaap}");
}

#[test]
fn fee_change_history_logged() {
    let mut engine = SimEngine::build_test(
        "history-test".into(), 123
    ).unwrap();

    engine.run_ticks(1).unwrap();

    // First fee change
    engine.submit_command(PlayerCommand::SetProductFee {
        product_id: "basic_checking".into(),
        fee_type:   "overdraft_fee".into(),
        new_value:  25.0,
    }).unwrap();
    ...
    assert_eq!(history[0].new_value, 30.0, "Most recent change should be $30");
...
fn determinism_holds_with_fee_changes() {
    ...
    let mut engine_a = SimEngine::build_test(
        format!("det-pricing-a-{SEED}"), SEED
    ).unwrap();
    let mut engine_b = SimEngine::build_test(
        format!("det-pricing-b-{SEED}"), SEED
    ).unwrap();
    ...
    let state_a = engine_a.store_product_state(&run_id_a, "basic_checking").unwrap();
    let state_b = engine_b.store_product_state(&run_id_b, "basic_checking").unwrap();
